...
@Component({
  ...
  styleUrls: ['./patient-transaction.component.css']
})
export class PatientTransactionComponent {
  ...
  ngOnInit():void {
    this.route.data.subscribe(data => {
      ...
      this.GetTransactions(this.patient);
    });
  ...
  GetTransactions(id: string){
    this.remitaService.PatientPayments(id, this.pagination, this.filters)
  ...
  downloadFile(pdfUrl: string) {
    ...
    link.href = pdfUrl;
    link.target = '_blank';
    link.download = pdfUrl.split('/').pop() || 'download__receipt.pdf'; // Extract the filename or set a default
